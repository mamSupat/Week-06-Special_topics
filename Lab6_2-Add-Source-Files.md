...
ตอบ การแยก source code เป็นหลายไฟล์ช่วยให้โค้ดชัดเจน อ่านง่าย และบำรุงรักษาง่ายขึ้น
2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
ตอบ เมื่อเพิ่มไฟล์ source ใหม่ ต้องแก้ไขไฟล์ CMakeLists.txt โดยเพิ่มชื่อไฟล์ใหม่ในส่วน SRCS ถ้ามี header ใหม่อยู่นอก directory ปัจจุบัน ให้เพิ่ม path ใน INCLUDE_DIRS ถ้าใช้ external components ให้ระบุใน REQUIRES หรือ PRIV_REQUIRES
3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
ตอบ ประกาศ function prototypes, constants, และ data structures เพื่อให้ไฟล์อื่นเรียกใช้ได้ ทำหน้าที่เป็น interface ระหว่างโมดูล โดยไม่ต้องเปิดดู implementation ใน .c ช่วยให้ compiler ตรวจสอบ type safety และให้ linker รู้ว่าต้อง link ฟังก์ชันจากไฟล์ไหน
4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
ตอบ INCLUDE_DIRS ใน CMakeLists.txt บอก compiler ให้รู้ว่าจะค้นหา header files ที่ไหน
5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
ตอบ ไฟล์ .gitignore ป้องกันไม่ให้ไฟล์ที่ไม่จำเป็นหรือสร้างอัตโนมัติถูก commit เข้า repository
6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
ตอบ LED ทำงานแบบ non-blocking (กระพริบ/เปลี่ยนสีต่อเนื่องโดยไม่หยุด tasks อื่น) ใช้ vTaskDelay เพื่อให้ระบบ schedule tasks อื่นในช่วงรอ และควบคุม LED หลายดวงได้พร้อมกันอย่างอิสระ
7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
ตอบ แต่ละไฟล์มีหน้าที่ชัดเจน, Maintainability สูง แก้ไขได้เฉพาะส่วนที่เกี่ยวข้อง, ทดสอบและ debug ได้ง่ายขึ้น, Reuse โมดูลได้ในโปรเจ็กต์อื่น, ทำงานเป็นทีมได้มีประสิทธิภาพ เพราะแบ่งความรับผิดชอบชัดเจน
...
- จำนวนไฟล์ source: 1 ไฟล์
- ขนาด binary: 85 bytes
- การทำงาน: อ่านค่าจากเซนเซอร์และแสดงผลผ่าน serial monitor

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 2 ไฟล์
- ขนาด binary: 97 bytes
- การทำงาน: อ่านค่าจากเซนเซอร์แล้วแสดงบนจอ LCD/OLED

**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source: 3 ไฟล์
- ขนาด binary: 103 bytes
- การทำงาน: อ่านค่าจากเซนเซอร์, แสดงบนจอ, และควบคุม LED ให้เปลี่ยนสถานะตามเงื่อนไข
